...
def getLatestTweet(tid):
    ...
    api = tweepy.API(auth)
    username=str(tid)
    tweet_list= api.user_timeline(username, count=10)
    t.clear()
    for x in tweet_list:
        #print(x.text)
        t.append(x.text)
...
def depdet_post():
    symbol = request.form['tid']
    quote = getLatestTweet(symbol)
    senti= datreeINPUT(quote)
    tw = sendtweet()
    return render_template('second.html', len = len(tw), tw = tw, senti = senti)
